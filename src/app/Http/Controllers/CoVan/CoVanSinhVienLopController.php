...
class CoVanSinhVienLopController extends Controller
{
    public function index($maSSV)
    {
        // 1. BỔ SUNG: Tìm kiếm thông tin sinh viên từ Database để lấy họ tên
        $sinhVienCheck = DB::table('sinh_vien')
            ->where('maSSV', $maSSV)
            ->where('isDelete', false)
            ->first();

        // Nếu không tồn tại sinh viên mang mã này, chặn lại báo lỗi 404
        if (!$sinhVienCheck) {
            abort(404, 'Không tìm thấy thông tin sinh viên mang mã số này.');
        }

        // Định nghĩa biến $tenSV từ dữ liệu DB để hàm compact() ở cuối không bị báo lỗi sập trang
        $tenSV = ($sinhVienCheck->HoSV ?? '') . ' ' . ($sinhVienCheck->TenSV ?? ($sinhVienCheck->hoTen ?? ''));
        ...
        return view('covan.hockysv', compact(
